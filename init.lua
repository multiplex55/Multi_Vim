...
require('lazy').setup({
  ...
  },
  {
    ...
    vim.keymap.set('n', '<space>fb', ':Telescope file_browser path=%":p:h select_buffer=true<CR>', { desc = 'Telescope [f]ile [b]rowser' }),
  },
  {
    'jiaoshijie/undotree',
  ...
  },
  {
    vim.keymap.set('n', '<leader>npr', function()
      ...
      local full_path_with_ext = vim.fn.expand '%:p' -- full path with extension
      ...
      -- Execute the command
      vim.cmd(cmd)
    end, { desc = '[n]im c[p]p [r]un release' }),
  },
  {
    ...
    config = function()
      ...
      end, { desc = 'ctrl + Quick M[e]nu' })

      vim.keymap.set('n', '<C-h>', function()
        harpoon:list():select(1)
      end, { desc = 'ctrl + [h]first harpoon' })

      --vim.keymap.set('n', '<C-t>', function()
      --harpoon:list():select(2)
      --end, { desc = 'ctrl + [t]second harpoon' })
